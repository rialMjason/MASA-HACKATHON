...
# Filter features to those with sufficient data
...
print(f"✓ Filtered to {len(feature_indicators_filtered)} features with {min_data_points}+ data points")
...
if X.shape[1] > max_features_for_analysis:
    ...
    print(f"✓ Reduced to {X.shape[1]} features with highest variance for computational efficiency")
...
def calculate_metrics(X_subset, y, feature_indices):
    ...
    try:
        ...
        k = min(5, n // 3)  # Ensure enough samples per fold
...
max_features = min(12, X.shape[1])  # Limit to 12 features to avoid explosion
...
for p in range(1, max_features + 1):
    print(f"\nTesting {p}-predictor models...", end=" ", flush=True)
    ...
    if len(all_combos) > max_combos_per_size:
        ...
        print(f"(testing {len(combos)}/{len(all_combos)} combinations)", end=" ")
    else:
        combos = all_combos
    ...
    if best_model_for_size:
        print(f"✓ {count} models | Best BIC: {best_bic_for_size:.2f}")
    else:
...
for idx, row in top_10.iterrows():
    print(f"{idx+1:2d}. Predictors: {int(row['n_predictors']):2d} | " +
          f"CV-RMSE: {row['cv_rmse']:10.4f} | AIC: {row['aic']:10.2f} | " +
          f"AICc: {row['aicc']:10.2f} | BIC: {row['bic']:10.2f} | R²: {row['r_squared']:.4f}")
...
print(f"   Indicators:")
for i, feat in enumerate(best_cv['features'], 1):
    print(f"     {i}. {feat}")
...
print(f"   Indicators:")
for i, feat in enumerate(best_aic['features'], 1):
    print(f"     {i}. {feat}")
...
print(f"   Indicators:")
for i, feat in enumerate(best_aicc['features'], 1):
    print(f"     {i}. {feat}")
...
# Group by number of predictors, find best R² and BIC for each size
...
for _, row in pareto.iterrows():
    print(f"│  {int(row['n_predictors']):2d}   │ {row['r_squared']:.4f} │ {row['cv_rmse']:10.4f} │ {row['bic']:9.2f} │")
print("└────────┴─────────┴────────────┴───────────┘")
...
Interpretation:
  ...
  (not overfitted) as validated by cross-validation.
""")
...
with open('model_outputs/best_indicators_summary.txt', 'w') as f:
    ...
    f.write(f"\nTop 10 Models:\n")
    for idx, row in top_10.iterrows():
        f.write(f"{idx+1:2d}. Predictors: {int(row['n_predictors']):2d} | " +
                f"CV-RMSE: {row['cv_rmse']:10.4f} | AIC: {row['aic']:10.2f} | " +
                f"AICc: {row['aicc']:10.2f} | BIC: {row['bic']:10.2f} | R²: {row['r_squared']:.4f}\n")
